...
public class BuildingFloorRateService {
    ...
    public List<BuildingFloorRate> getAll() {
        return repository.findAll();
    }

    public BuildingFloorRate getById(Long id) {
    ...
    public BuildingFloorRate create(
    ...
    public BuildingFloorRate update(
            Long id,
            BuildingFloorRateRequest request) {

        BuildingFloorRate entity = getById(id);
        ...
        boolean isSameRecord = entity.getBuildingType().getId()
        ...
        if (exists && !isSameRecord) {
            throw new RuntimeException(
                    "Building floor rate already exists for the "
                            + "specified building/floor/component combination");
        }
